...
func (cq *CompanyQuery) QueryBots() *BotQuery {
	query := (&BotClient{config: cq.config}).Query()
...
func (cq *CompanyQuery) WithBots(opts ...func(*BotQuery)) *CompanyQuery {
	query := (&BotClient{config: cq.config}).Query()
...
func (cq *CompanyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Company, error) {
	...
	if query := cq.withBots; query != nil {
		if err := cq.loadBots(ctx, query, nodes,
			func(n *Company) { n.Edges.Bots = []*Bot{} },
			func(n *Company, e *Bot) { n.Edges.Bots = append(n.Edges.Bots, e) }); err != nil {
			return nil, err
		}
	}
...
func (cq *CompanyQuery) loadBots(ctx context.Context, query *BotQuery, nodes []*Company, init func(*Company), assign func(*Company, *Bot)) error {
	...
	query.Where(predicate.Bot(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(company.BotsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.company_bots
		if fk == nil {
			return fmt.Errorf(`foreign-key "company_bots" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "company_bots" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (cq *CompanyQuery) loadUsers(ctx context.Context, query *UserQuery, nodes []*Company, init func(*Company), assign func(*Company, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(company.UsersPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(company.UsersPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(company.UsersPrimaryKey[1]))
