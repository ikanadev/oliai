...
func (cc *CompanyCreate) SetArchivedAt(t time.Time) *CompanyCreate {
	cc.mutation.SetArchivedAt(t)
	return cc
}
...
func (cc *CompanyCreate) AddBotIDs(ids ...uuid.UUID) *CompanyCreate {
	cc.mutation.AddBotIDs(ids...)
	return cc
}

// AddBots adds the "bots" edges to the Bot entity.
func (cc *CompanyCreate) AddBots(b ...*Bot) *CompanyCreate {
	ids := make([]uuid.UUID, len(b))
	for i := range b {
		ids[i] = b[i].ID
	}
	return cc.AddBotIDs(ids...)
}
...
func (cc *CompanyCreate) check() error {
	...
	if _, ok := cc.mutation.ArchivedAt(); !ok {
		return &ValidationError{Name: "archived_at", err: errors.New(`ent: missing required field "Company.archived_at"`)}
	}
	return nil
}
